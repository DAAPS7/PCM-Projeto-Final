...
button = html `<button class="btn-secondary" disabled>Remove Song</button>`; 
...
app = {
  ...
  const { AudioVisualization } = await import("/assets/js/AudioVisualization.js");
  const { ParticleVisualization } = await import("/assets/js/ParticleVisualization.js");
  const { ReactiveBallVisualization } = await import("/assets/js/ReactiveBallVisualization.js");
  const { SpectrumVisualization } = await import("/assets/js/SpectrumVisualization.js");
  const { WaveformVisualization } = await import("/assets/js/WaveformVisualization.js");

  return new App({
    audioEl,
    audioContext,
    canvasId: "audioCanvas"
  });
}
...
removeSong = {
  button.addEventListener("click", (e) => {
    ...
    // Trigger the reactive reset
    mutable resetSignal = resetSignal + 1;
    
    // Re-enable the upload input
    document.getElementById("upload-input").disabled = false;
    button.disabled = true;
  });
}

feedback = html` <section id="feedback">
  <p class="text-feedback">Are the song and lyrics correct?</p>
</section>`;

checkButton = html`<button class="check" disabled><i class="fa-solid fa-check"></i></button>`; 
crossButton = html`<button class="cross" disabled><i class="fa-solid fa-xmark"></i></button>`;

feedbackButtons = {
  feedback.appendChild(checkButton);
  feedback.appendChild(crossButton);
...
mediaButtonsEvents = {
    ...
    playButton.onclick = () => {
        ...
        songPlaying ? app.play() : app.pause();
...
visualizationSelection = {
  visualizationDropdown.addEventListener("change", (e) => {
    app.setVisualization(e.target.value);
  });
}
